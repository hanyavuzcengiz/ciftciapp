...
- `PAYMENT_PROVIDER_MODE`: `mock` (default) veya `live`; production için varsayılan güvenli mod `live` zorunludur (`PAYMENT_ALLOW_MOCK_PROVIDER_IN_PROD=1` verilmedikçe). `live` modda ilgili sağlayıcı için `PAYMENT_IYZICO_*` / `PAYMENT_STRIPE_*` değişkenleri birlikte verilmelidir.
- `PAYMENT_RECONCILIATION_TOKEN`: set edilirse `POST /payments/reconciliation/report` endpointi `x-reconciliation-token` ister; mismatch oranı `PAYMENT_RECONCILIATION_ALERT_RATIO` eşiğini aşarsa `reconciliation_mismatch_alert` logu üretilir.
- `PAYMENT_RECONCILIATION_ALERT_WEBHOOK_URL`: alert üretildiğinde JSON payload bu URL'e POST edilir; scheduler için `PAYMENT_RECONCILIATION_SOURCE_URL`, `PAYMENT_RECONCILIATION_SOURCE_TOKEN`, `PAYMENT_RECONCILIATION_SCHEDULE_MS` değişkenleri kullanılabilir.
